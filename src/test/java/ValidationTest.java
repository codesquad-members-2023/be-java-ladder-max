...
public class ValidationTest {
    ...
    void checkValidName_test() {
        List<String> names = new ArrayList<>();
        names.add("abc");
        assertThat(checkValidName(Optional.of(names))).isTrue();
        names.add("abc"); // 중복 이름
        assertThat(checkValidName(Optional.of(names))).isFalse();
        names.clear();
        names.add("춘식이"); // 종료 이름
        assertThat(checkValidName(Optional.of(names))).isFalse();
    ...
    void checkValidNum_test() {
        String input = ""; // 빈 문자열
        assertThat(checkValidNumber(Optional.of(input))).isFalse();
        input = "-1"; // 음수
        assertThat(checkValidNumber(Optional.of(input))).isFalse();
        input = "abc"; // 숫자가 아닌 문자
        assertThat(checkValidNumber(Optional.of(input))).isFalse();
        ...
        assertThat(checkValidNumber(Optional.of(input))).isFalse();
        input = "3";
        assertThat(checkValidNumber(Optional.of(input))).isTrue();
    }
    ...
    void checkValidResults_test() {
        ...
        assertThat(checkValidResults(Optional.of(results), 1)).isFalse();
        results.clear();
        assertThat(checkValidResults(Optional.of(results), 1)).isFalse(); // 결과와 참여한 사람들의 수가 다를 때
    }
}
