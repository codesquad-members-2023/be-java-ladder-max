...
- [ ] 플레이어 인원 수 최소 2명 이상 입력하도록 예외처리
- [x] 쉼표를 기준으로 자르기
- [x] 플레이어 이름을 ArrayList에 저장하기
- [x] 최대 사다리 높이 입력 받기
- [ ] 플레이어 인원과 최대 사다리 높이를 가지고 사다리를 생성한다.
  - [x] Ladder 클래스에서 최소 사다리 높이(1이상 입력하도록 제한)를 예외처리한다.
  - [x] 랜덤한 다리를 boolean으로 표현하는 RandomLine enum 클래스를 만든다.
  - [x] Ladder 클래스에서 randomLines를 생성한다.
    - 각 행의 같은 인덱스에 연속된 다리가 생기지 않도록 제한하기
    - 각 행에 최소한 1개의 다리가 생성되도록 제한하기
